...
class CLaMP(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if hasattr(audio_outputs, 'last_hidden_state') and audio_outputs.last_hidden_state is not None:
            ...
            audio_features = audio_outputs.last_hidden_state  # Shape: (batch_size, 768, 2, 32)

            # Global average pooling over the last two dimensions
            audio_features = audio_features.mean(dim=(-2, -1))  # Shape: (batch_size, 768)
            print(f"Reduced audio_features shape: {audio_features.shape}")

            audio_embeddings = self.audio_proj(audio_features)  # Shape: (batch_size, embedding_dim)
        else:
        ...
        if hasattr(text_outputs, 'last_hidden_state') and text_outputs.last_hidden_state is not None:
            text_features = text_outputs.last_hidden_state.mean(dim=1)  # Average over sequence length
        ...
        # Combine embeddings
        combined_embeddings = torch.cat((audio_embeddings, melody_embeddings, text_embeddings), dim=1)  # Concatenate along feature dimension
